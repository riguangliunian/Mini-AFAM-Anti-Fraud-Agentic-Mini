...
one action per round. Follow the standard investigation flow:
  Round 1: expand_neighbors (gather context)
  Round 2: analyze / generate_rule when patterns emerge
  Round 3: shadow_replay to validate
  Round 4: adversarial_probe (red-team the rule)
  Round 5+: terminate with verdict
...
class OrchestratorConfig:
    ...
    use_alignment: bool = True  # 关掉时 system prompt 会去掉 preferences 段
    use_rules: bool = True
...
class Orchestrator:
    ...
    def investigate(self, alert: dict) -> Trajectory:
        ...
        for round_i in range(1, self.config.max_rounds + 1):
            ...
            traj.steps.append(TrajectoryStep(state=self._snapshot(state),
                                              action=action,
                                              outcome=outcome))
            state.action_history_summary.append(action.short())

            # 7. 终止判定
            if action.action_type == "terminate":
                traj.verdict = action.params.get("verdict", "unknown")
        ...
        traj.label = self._auto_label(traj)  # 自动打 accepted/rejected 标签用于后续 DPO
    ...
    def _system_prompt(self) -> str:
        ...
        if not self.config.use_alignment:
            ...
            prompt = prompt.split("# Preferences")[0].rstrip() + "\n\n" + prompt.split("# Available actions")[1]
            prompt = "# Available actions" + prompt.split("# Available actions")[-1] \
                     if "# Available actions" in prompt else prompt
        return prompt
    ...
    def _auto_label(self, traj: Trajectory) -> str:
        """
        依据真实标签评估:调查判定是否正确。
        用于事后打 accepted/rejected 供 DPO 训练。
        """
        # 检查 seed_user 的真实 label(简化的评估)
        # 生产里应该是人工复核
        if traj.verdict == "fraud_confirmed" and traj.final_confidence >= 0.7:
            return "accepted"
        if traj.verdict == "escalate":
            return "accepted"  # 谨慎升级也是好行为
        if traj.verdict == "not_fraud":
            return "accepted"
        return "rejected"
