...
# --------------------------------------------------------------------------------------
# OpenAI-compatible backend(支持 OpenAI API / yunwu / Ollama 本地)
# --------------------------------------------------------------------------------------
...
class OpenAILLM:
    ...
    def chat(self, messages: list[dict[str, str]]) -> str:
        ...
        # Ollama 目前 JSON mode 支持不完整,不强制;OpenAI/yunwu 强制 JSON
...
def get_llm():
    ...
    - LLM_MODEL=qwen-plus + OPENAI_BASE_URL=https://yunwu.ai/v1        → yunwu 中转
    """
