...
class RuleStream:
    ...
    def _check_expand_neighbors(self, action: Action, state: State) -> Optional[Violation]:
        ...
        if edge_type == "contact" and hop >= 2 and state.round_num == 0:
            return Violation(
                "PRIVACY_GUARD",
    ...
    def _check_generate_rule(self, action: Action, state: State) -> Optional[Violation]:
        ...
        has_attribute_signal = any(kw in pattern.lower() for kw in
                                    ["new_account", "age", "amount", "night", "speed",
                                     "paste", "black_list"])
        if has_structure_signal and not has_attribute_signal:
            return Violation(
                "STRUCTURE_ONLY_RULE",
                f"Rejected: rule pattern uses only structural signal ({pattern!r}). "
                "Must combine at least one attribute signal (new_account/night/etc) "
