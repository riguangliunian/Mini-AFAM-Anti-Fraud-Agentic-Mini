...
1. `src/` 原有单案/团伙调查 Agent；
2. `src/model_refresh/` 新增反欺诈 GNN 效果衰退诊断与模型刷新 Agent。
3. `src/production_diagnosis/` 新增线上反欺诈模型诊断与优化 Agent。

模型刷新 Agent 面向“节点特征漂移、图结构漂移、标签延迟、数据管道故障、业务分群迁移、新型攻击”六类事件，在有限实验预算下执行：

```text
监控触发 → 定位衰退分群 → 诊断根因 → 选择定向修复
→ 训练候选GNN → 时间外测试 → Shadow验证 → 发布/回滚建议
```

快速运行：

```bash
LLM_MODEL=mock python -m experiments.evaluate_refresh --modes baseline retrieval
python -m experiments.build_refresh_dpo_data
```

Mock结果只验证Agent工作流和护栏是否正确，不能作为真实GNN效果结论。真实训练需实现`SimulatedGNNModelLab`相同接口，详见`docs/model_refresh.md`。

生产诊断 Agent 的目标不是自动训练模型，而是模拟资深反欺诈算法工程师在模型监控告警后的完整排查流程：

```text
Model Alert -> Planner -> SQL/Data Quality/PSI/SHAP/Behavior/Graph/Replay Tools
            -> Root Cause Hypothesis -> Repair Strategy
```

快速运行：

```bash
LLM_MODEL=mock python -m experiments.evaluate_diagnosis --modes baseline retrieval
python -m experiments.build_diagnosis_dpo_data
```

说明见`docs/production_diagnosis.md`。
